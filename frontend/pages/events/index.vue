...
<template>
  <div>
    ...
    <p v-else-if="!data?.items.length" class="text-sm text-muted-foreground">No events found.</p>
    <div v-else class="space-y-1">
      <details v-for="e in data.items" :key="e.event_id" class="rounded border border-border p-2 text-sm">
        <summary class="cursor-pointer flex items-center gap-2 flex-wrap">
          ...
          <span class="text-xs text-muted-foreground">{{ new Date(e.timestamp).toLocaleString() }}</span>
        </summary>
