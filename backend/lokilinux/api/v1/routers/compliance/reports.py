...
async def create_report(
    ...
    background_tasks.add_task(_run_report_generation, session_factory, storage, report.id)

    return ComplianceReportResponse.model_validate(report)


async def _run_report_generation(session_factory, storage, report_id: UUID) -> None:
    async with session_factory() as db:
        ...
        if report is None:
            return
        await generate_report(db, storage, report)
...
async def download_report(
    ...
    if report.status != "COMPLETED" or (report.body is None and report.storage_object_id is None):
        raise HTTPException(
    ...
    # Dual-read: reports generated after the Object Storage plan landed carry
    # storage_object_id; older rows still have the artifact inline in `body`.
    if report.storage_object_id is not None:
        storage = request.app.state.storage
        service = StorageService(storage, db)
        _obj, stream = await service.open_stream(report.storage_object_id)
        return StreamingResponse(stream, media_type=media_type, headers=headers)

    return Response(content=report.body, media_type=media_type, headers=headers)
